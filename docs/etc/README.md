...
- **`results/`** — banked coverage/localization measurement runs (JSON) with their
  `PROVENANCE.md` recording how each number was produced.
- **`PR_cleaning-jobs_README.md`, `PR_nav-localize_README.md`** — the original
  contribution PR-body drafts for the `cleaning-jobs` and `nav-localize` modules in
