...
localization_health and global_relocalizer are pure mechanism -- they report
"you are lost" and "here is the best pose and how confident I am". This node
owns the POLICY: on /localization_lost it calls the relocalizer, then on the
result decides whether to trust the fix (commit it to /initialpose) or fall
back to a recovery behavior (drive around, hunt the dock via IR, ...). The
fallback is a STUB here -- it emits the chosen action on ~/recovery_action for a
real behavior node to carry out; actual motion / dock-IR handling comes later.
Keeping the decision here is why global_relocalizer never seeds /initialpose in
production (its publish_initialpose is debug only).

  sub  /localization_lost   std_msgs/Empty          (from localization_health)
...
class LocalizationManager(Node):
    ...
    def __init__(self) -> None:
        ...
        self.create_subscription(Empty, '/localization_lost', self._on_lost, 10)
        ...
        self.get_logger().info(
            'localization_manager: on lost -> relocalize -> commit if '
            'confidence >= %.2f, else %s' % (self.accept_conf, self.fallback))

    def _on_lost(self, _msg) -> None:
        if self.auto and self.state == NOMINAL:
            self.get_logger().warn('localization lost -- starting recovery')
            self._relocalize()
